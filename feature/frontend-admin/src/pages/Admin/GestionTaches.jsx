...
export default function GestionTaches() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setEmployes((Array.isArray(eData) ? eData : []).filter(u => u.role === "employe"));
      setPostes(Array.isArray(pData) ? pData : []);
      setRdvs(Array.isArray(rData) ? rData : []);
    } catch (e) {
      setError("Impossible de charger les données.");
    } finally {
      setLoading(false);
                ...
                <tbody>
                  {filtered.map((t, i) => (
                    <tr key={t.id} style={{ ...s.tr, background: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                      ...
                      <td style={s.td}>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button style={s.iconBtn} title="Détail"
                            onClick={() => handleDetail(t)}>👁️</button>
                          <button style={s.iconBtn} title="Modifier"
                            onClick={() => handleEdit(t)}>✏️</button>
                          <button style={s.iconBtn} title="Commentaires"
                            onClick={() => navigate(`/taches/${t.id}/commentaires`)}>💬</button>
                          <button style={{ ...s.iconBtn, ...s.iconBtnDanger }} title="Supprimer"
                            onClick={() => setDeleteId(t.id)}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  ))}
